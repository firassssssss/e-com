...
//   Users, RefreshCw, Search, ShieldAlert, ChevronRight, Clock,
// } from "lucide-react";
...
//   if (c == null) return "bg-gray-100 text-gray-400";
...
// }) {
//   return (
//     <div className={`bg-white p-5 rounded-sm border ${warn ? "border-red-200" : "border-[#E0D5C8]"}`}>
//       <div className="flex items-center justify-between mb-2">
//         <span className="text-xs text-[#6B4F3A]/60 uppercase tracking-wide">{label}</span>
//         <Icon size={16} className={warn ? "text-red-400" : "text-[#C4786A]"} />
//       </div>
//       <div className={`text-2xl font-light ${warn ? "text-red-600" : "text-[#1A1410]"}`}>{value}</div>
//       {sub && <div className="text-xs text-[#6B4F3A]/50 mt-1">{sub}</div>}
//     </div>
//   );
// }
...
//   return (
//     <div className="bg-white rounded-sm border border-[#E0D5C8] p-5">
//       <h3 className="text-sm font-medium text-[#1A1410] mb-4">Messages / hour (24h)</h3>
//       <div className="flex items-end gap-1 h-16">
//         {data.map(d => (
//           <div key={d.hour} className="flex-1 flex flex-col items-center gap-1 group relative">
//             <div
//               className="w-full bg-[#C4786A]/30 hover:bg-[#C4786A]/60 transition-colors rounded-t-sm"
...
//                             bg-[#1A1410] text-white text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap z-10">
//               {d.count} · {new Date(d.hour).getHours()}:00
//             </div>
//           </div>
//         ))}
//       </div>
...
//   user: ChatUser; selected: boolean; onClick: () => void;
// }) {
//   return (
//     <button
//       onClick={onClick}
//       className={`w-full text-left px-4 py-3 border-b border-[#E0D5C8] transition-colors
//         ${selected ? "bg-[#F0EAE2]" : "bg-white hover:bg-[#FAF7F4]"}`}
//     >
...
//           )}
//           <span className={`text-sm font-medium truncate ${user.isAnon ? "text-[#6B4F3A]/60 italic" : "text-[#1A1410]"}`}>
//             {user.identifier}
//           </span>
//         </div>
//         <ChevronRight size={14} className={`shrink-0 mt-0.5 ${selected ? "text-[#C4786A]" : "text-[#E0D5C8]"}`} />
//       </div>
//       <div className="flex items-center gap-3 mt-1 text-xs text-[#6B4F3A]/50">
//         <span>{user.messageCount} msg{user.messageCount !== 1 ? "s" : ""}</span>
//         <span>·</span>
...
//         </span>
//         {user.hasSuspicious && (
//           <span className="ml-auto text-red-400 font-medium">⚠ suspicious</span>
//         )}
//       </div>
//     </button>
//   );
...
//   return (
//     <div className={`px-5 py-4 border-b border-[#E0D5C8]/60 ${log.suspicious ? "bg-red-50/40" : ""}`}>
//       {/* Timestamp + flags */}
//       <div className="flex items-center gap-2 mb-2 text-[10px] text-[#6B4F3A]/40 uppercase tracking-wide">
//         <span>{fmtDate(log.createdAt)}</span>
//         {log.intent && (
//           <span className="bg-[#F0EAE2] text-[#6B4F3A] px-2 py-0.5 rounded-full">{log.intent}</span>
//         )}
//         {log.suspicious && (
//           <span className="bg-red-100 text-red-500 px-2 py-0.5 rounded-full flex items-center gap-1">
...
//             <div className="bg-[#F0EAE2] text-[#1A1410] px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm leading-relaxed">
//               {botMsg.text}
//             </div>
//             <div className="flex items-center gap-2 px-1">
//               {conf != null && (
//                 <span className={`text-[10px] px-2 py-0.5 rounded-full ${confColor(conf)}`}>
//                   conf {(conf * 100).toFixed(0)}%
//                 </span>
//               )}
//               <span className="text-[10px] text-[#6B4F3A]/30">id: {log.id.slice(0, 8)}…</span>
//             </div>
//           </div>
//         </div>
//       )}
...
//   // ── Fetch health metrics (auto-refresh 10s) ──
...
//         const logs: ConvLog[] = (r.data.data ?? []).reverse();
...
//   const displayedConv = suspiciousOnly
//     ? conversation.filter(l => l.suspicious)
//     : conversation;
...
//     return (
//       <div className="flex items-center justify-center h-64 text-[#6B4F3A]/50">
...
//   return (
//     <div className="h-screen flex flex-col bg-[#FAF7F4] overflow-hidden">

//       {/* ── Header ── */}
//       <div className="px-8 pt-7 pb-5 border-b border-[#E0D5C8] bg-white flex items-center justify-between shrink-0">
//         <div>
//           <h1 className="text-2xl font-light text-[#1A1410]">Chatbot Monitor</h1>
//           <p className="text-xs text-[#6B4F3A]/50 mt-0.5">Live · last 24 hours · auto-refresh 10s</p>
//         </div>
//         <button
//           onClick={() => { fetchHealth(); fetchUsers(); }}
//           className="flex items-center gap-2 text-xs text-[#6B4F3A]/60 hover:text-[#6B4F3A]
//                      border border-[#E0D5C8] rounded-sm px-3 py-1.5 transition-colors"
//         >
...
//       {/* ── Metrics row ── */}
//       {health && (
//         <div className="px-8 py-5 grid grid-cols-6 gap-4 shrink-0">
//           <MetricCard
//             label="Conversations"
//             value={health.totalConversations}
//             icon={MessageSquare}
//             sub="past 24h"
//           />
//           <MetricCard
//             label="Unique chatters"
//             value={health.uniqueUsers}
//             icon={Users}
//             sub={`${health.anonSessions} anon`}
//           />
//           <MetricCard
//             label="Avg confidence"
//             value={`${(health.avgConfidence * 100).toFixed(1)}%`}
//             icon={Activity}
//             sub={`${health.avgMessagesPerUser.toFixed(1)} msg/user`}
//           />
//           <MetricCard
//             label="Hallucination est."
//             value={`${health.hallucinationRate.toFixed(1)}%`}
//             icon={AlertTriangle}
//             warn={health.hallucinationRate > 10}
//           />
//           <MetricCard
//             label="Negative feedback"
//             value={`${health.negativeFeedbackRate.toFixed(1)}%`}
//             icon={ThumbsDown}
//             sub={`${health.totalFeedbacks} total`}
//             warn={health.negativeFeedbackRate > 20}
//           />
//           <MetricCard
//             label="Positive feedback"
//             value={`${health.positiveFeedbackRate.toFixed(1)}%`}
//             icon={ThumbsUp}
//           />
//         </div>
//       )}
...
//       {/* ── Two-panel body ── */}
//       <div className="flex flex-1 min-h-0 mx-8 mb-6 gap-4">

//         {/* Left: User list */}
//         <div className="w-72 shrink-0 bg-white border border-[#E0D5C8] rounded-sm flex flex-col overflow-hidden">

//           {/* Search + filter */}
//           <div className="p-3 border-b border-[#E0D5C8] space-y-2">
//             <div className="relative">
//               <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#6B4F3A]/40" />
//               <input
//                 value={search}
//                 onChange={e => setSearch(e.target.value)}
//                 placeholder="Search user / session…"
//                 className="w-full pl-8 pr-3 py-1.5 text-xs border border-[#E0D5C8] rounded-sm
//                            bg-[#FAF7F4] text-[#1A1410] placeholder:text-[#6B4F3A]/40
//                            focus:outline-none focus:border-[#C4786A]"
//               />
//             </div>
...
//                     ${userFilter === f
//                       ? "bg-[#1A1410] text-white"
//                       : "bg-[#F0EAE2] text-[#6B4F3A] hover:bg-[#E0D5C8]"
//                     }`}
//                 >
...
//             {usersLoading ? (
//               <div className="p-4 text-xs text-[#6B4F3A]/40 text-center">Loading users…</div>
//             ) : filteredUsers.length === 0 ? (
//               <div className="p-4 text-xs text-[#6B4F3A]/40 text-center">No users found</div>
//             ) : (
//               filteredUsers.map(u => (
//                 <UserCard
...
//           {/* Footer count */}
//           <div className="px-4 py-2 border-t border-[#E0D5C8] text-[10px] text-[#6B4F3A]/40">
//             {filteredUsers.length} of {users.length} users
//           </div>
//         </div>

//         {/* Right: Conversation panel */}
//         <div className="flex-1 bg-white border border-[#E0D5C8] rounded-sm flex flex-col overflow-hidden">

//           {!selectedUser ? (
//             // Empty state
//             <div className="flex-1 flex flex-col items-center justify-center text-[#6B4F3A]/40 gap-3">
//               <MessageSquare size={32} strokeWidth={1} />
//               <p className="text-sm">Select a user to inspect their conversation</p>
//             </div>
//           ) : (
//             <>
//               {/* Conv header */}
//               <div className="px-5 py-3.5 border-b border-[#E0D5C8] flex items-center justify-between shrink-0">
//                 <div>
//                   <div className="flex items-center gap-2">
//                     {selectedUser.hasSuspicious && (
//                       <ShieldAlert size={14} className="text-red-400" />
//                     )}
//                     <span className={`text-sm font-medium ${selectedUser.isAnon ? "italic text-[#6B4F3A]/60" : "text-[#1A1410]"}`}>
//                       {selectedUser.identifier}
//                     </span>
//                     {selectedUser.isAnon && (
//                       <span className="text-[10px] bg-[#F0EAE2] text-[#6B4F3A] px-2 py-0.5 rounded-full">anon</span>
//                     )}
//                   </div>
//                   <p className="text-xs text-[#6B4F3A]/50 mt-0.5">
//                     {selectedUser.messageCount} messages · last active {timeAgo(selectedUser.lastActive)}
//                   </p>
//                 </div>

//                 <label className="flex items-center gap-2 text-xs text-[#6B4F3A]/60 cursor-pointer select-none">
//                   <input
//                     type="checkbox"
...
//               <div className="flex-1 overflow-y-auto">
//                 {convLoading ? (
//                   <div className="flex items-center justify-center h-32 text-[#6B4F3A]/40 text-sm">
//                     <RefreshCw size={16} className="animate-spin mr-2" /> Loading conversation…
//                   </div>
...
//                 <div className="px-5 py-2 border-t border-[#E0D5C8] text-[10px] text-[#6B4F3A]/40 flex gap-4 shrink-0">
//                   <span>{displayedConv.length} turns shown</span>
//                   <span>{displayedConv.filter(l => l.suspicious).length} suspicious</span>
//                   {displayedConv[0] && (
//                     <span>first: {fmtDate(displayedConv[0].createdAt)}</span>
//                   )}
//                 </div>
//               )}
//             </>
