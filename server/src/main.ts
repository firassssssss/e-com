...
async function checkServicesHealth(): Promise<void> {
  ...
  try {
    ...
    console.warn(`⚠️  Ollama NOT reachable at ${ollamaUrl} — chat will fail until Ollama is running`);
  }

  // Check RAG service
  try {
    const health = await ragClient.health();
    console.log(`✅ RAG service reachable at ${ragUrl} (${health.vectors} vectors indexed)`);
  } catch {
    console.warn(`⚠️  RAG service NOT reachable at ${ragUrl} — run: cd server/rag/service && uvicorn main:app --port 8001`);
  }
}
...
if (process.env.NODE_ENV !== 'test') {
  const server = app.listen(port, () => {
    ...
    // Non-blocking service health check — results appear a moment after startup
    checkServicesHealth().catch(() => {});
  });
