...
function buildRagClient(): AxiosInstance {
  ...
  if (!process.env.RASA_SERVICE_TOKEN) {
    console.warn('[RagClient] RASA_SERVICE_TOKEN is not set — all RAG requests will be rejected with 401.');
  }

  instance.interceptors.request.use((config) => {
    const token = process.env.RASA_SERVICE_TOKEN;
...
export const ragClient = {
  ...
  async search(query: string, limit = 4, timeoutMs?: number): Promise<{ results: any[] }> {
  ...
  async classifyInjection(message: string): Promise<{ is_injection: boolean; score: number; threshold: number }> {
    try {
