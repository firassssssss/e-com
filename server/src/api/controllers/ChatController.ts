...
- Do NOT proactively push products into every response. Only bring up specific products when the user asks for a recommendation or asks what to buy.
- When the user asks for a recommendation: immediately name 1-2 specific products from the PERMITTED list by their exact name and price. Do NOT ask for more clarification if you already know their skin type from the conversation.
  ...
  async message(@Body() body: ChatMessageDto, @CurrentUser() currentUser: any, @Res() res: Response) {
    ...
    try {
      ...
      try {
        ...
        const ragRes = await ragClient.search(ragQuery, 6, 5000);
        const products = ragRes?.results;
        if (Array.isArray(products) && products.length) {
          hadRagResults  = true;
          productContext = `\nPERMITTED PRODUCTS — use these when the user asks for a recommendation:\n` +
      ...
      } catch (e) { console.warn("[CHAT] RAG unavailable:", (e as Error).message); }
