...
export class JwtService {
  ...
  generateToken(payload: { id: string; email: string }): string {
    ...
    const jti = uuidv4();
    return jwt.sign({ ...jwtPayload, jti }, JWT_SECRET!, signOptions);
  }
  ...
  static staticVerify(token: string): any {
    return jwt.verify(token, JWT_SECRET!);
  }

  static staticDecode(token: string): any {
    return jwt.decode(token);
  }

  sign(payload: Record<string, any>, secret: string, options: SignOptions): string {
    return jwt.sign({ ...payload, jti: uuidv4() }, secret, options);
  }
}
